...
$ConvertTheseVideos = @(
	$ConversionData0720x1280,
	$ConversionData1080x1920,
	$ConversionData2160x3840,
	$ConversionData2160x4096
)
...
Else {
	...
	ForEach ( $VideoObject in $ConvertTheseVideos ) {
		...
		If ( $VideoObject.ListOfInputVideos ) {
			...
			ForEach ( $Video in $VideoObject.ListOfInputVideos ) {
				...
				Write-Host "        Total Video Length: " -NoNewline
				...
				Write-Host "$CurrentVideoReadablewSize" -ForegroundColor DarkMagenta
				# Start a stopwatch
				...
				Write-Host "`r     Total Processing Time: " -NoNewline
	...
	Write-Host " All Videos Have Completed Processing "  -BackgroundColor DarkCyan -ForegroundColor White
}
################################################################################
# END: Work Section
################################################################################
